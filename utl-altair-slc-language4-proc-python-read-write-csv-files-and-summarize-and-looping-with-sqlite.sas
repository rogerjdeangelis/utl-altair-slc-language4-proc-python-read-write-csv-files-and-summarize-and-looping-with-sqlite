...
lines = sqldf("""
    SELECT line AS lines
    FROM split(readfile('d:/txt/class_export.txt'), char(10))
""")
print(lines)
conn.close()
endsubmit;
        ...
        (
            ...
            FROM (
                SELECT
                    line
                UNION ALL
                SELECT 'ID,Name,Age,Height'  -- Add header
                ORDER BY line DESC  -- This puts header first
                LIMIT -1 OFFSET 1   -- Adjust as needed
            )
        )
    )
""")
